...
from helix.config import WorkflowConfig, WorkflowMode
from helix.errors import StepMaxRetriesError, WorkflowError
...
class Workflow:
    ...
    def __init__(self, name: str, config: Optional[WorkflowConfig] = None) -> None:
        self._name = name
        self._config = config or WorkflowConfig(name=name)
        self._chain: List[Dict[str, Any]] = []  # List of {type, steps, kwargs}
        self._budget_usd: Optional[float] = config.budget_usd if config else None
    ...
    def then(self, step_fn: Union[Step, Callable], **kwargs: Any) -> "Workflow":
        """Add a sequential step."""
        s = step_fn if isinstance(step_fn, Step) else Step(fn=step_fn, name=getattr(step_fn, "__name__", "step"))
        self._chain.append({"type": "sequential", "step": s})
        return self

    def parallel(self, *step_fns: Union[Step, Callable]) -> "Workflow":
    ...
    def map(self, step_fn: Union[Step, Callable], items_fn: Callable) -> "Workflow":
        """Apply step_fn to each item returned by items_fn(input) concurrently."""
        s = step_fn if isinstance(step_fn, Step) else Step(fn=step_fn, name=getattr(step_fn, "__name__", "map_step"))
    ...
    def loop(
        self,
        step_fn: Union[Step, Callable],
        until: Callable[[Any], bool],
        max_iter: int = 10,
    ) -> "Workflow":
        """Repeat step_fn until until(output) is True or max_iter is reached."""
        s = step_fn if isinstance(step_fn, Step) else Step(fn=step_fn, name=getattr(step_fn, "__name__", "loop_step"))
    ...
    async def _execute_node(
        self, node: Dict[str, Any], current: Any
        ...
        if node_type == "sequential":
            ...
            return output, StepResult(name=s.name, output=output, duration_s=time.time() - node_start)
